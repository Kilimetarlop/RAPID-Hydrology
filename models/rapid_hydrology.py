...
from layers.RAPIDHydrology_EncDec import (
    ChannelIndependence,
...
from layers.Embed import Patch, PatchEmbedding, PositionalEncoding, TokenEmbedding_RAPIDHydrology


class FlattenHead(nn.Module):
    def __init__(
            self,
            input_seq_len: int,  # sequence length after patching
        ...
        input_dim = input_seq_len * d_model
        print(f"FlattenHead: input_dim={input_dim}, output_seq_len={output_seq_len}")

        self.forecast_head = nn.Linear(input_dim, output_seq_len)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        :param x: [batch_size, num_features, input_seq_len, d_model]
        :return: [batch_size, output_seq_len, num_features]
        """
        batch_size, num_features, input_seq_len, d_model = x.shape

        # Reshape to [batch_size, num_features, input_seq_len * d_model]
        x = x.reshape(batch_size, num_features, -1)

        # Apply linear layer [batch_size, num_features, output_seq_len]
        x = self.forecast_head(x)
        x = self.dropout(x)

        # Transpose to [batch_size, output_seq_len, num_features]
        x = x.permute(0, 2, 1)
        return x
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.seq_len = int((args.seq_len - self.patch_len) / self.stride) + 1
        # print(f"Actual input length: {args.seq_len}, Patch sequence length: {self.seq_len}")
        # Embedding
