...
class Exp_RAPIDHydrology(Exp_Basic):
    def __init__(self, args):
        super(Exp_RAPIDHydrology, self).__init__(args)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                for j in range(pred_np.shape[0]):
                    sample_data = {
                        'sample_id': len(all_predictions),
                        'batch_idx': i,
                        'sample_in_batch': j,
                        'input_data': input_np[j],
                        'predictions': pred_np[j],
                        'targets': true_np[j],
                    }
                    all_predictions.append(pred_np[j])
                    all_targets.append(true_np[j])
                    all_inputs.append(input_np[j])
                    all_metadata.append(sample_data)
        ...
        try:
            from utils.metrics import metric
            mae, mse, _, _, _ = metric(predictions_array, targets_array)
        except ImportError:
            print("Warning: cannot import metric function, computing MSE and MAE manually")
            mse = np.mean((predictions_array - targets_array) ** 2)
            mae = np.mean(np.abs(predictions_array - targets_array))

        print(f"Overall metrics - MSE: {mse:.4f}, MAE: {mae:.4f}")

        # Compute per-feature metrics
        n_features = predictions_array.shape[-1]
        feature_metrics = []

        for feature_idx in range(n_features):
            feature_pred = predictions_array[:, :, feature_idx]
            feature_true = targets_array[:, :, feature_idx]

            feature_mse = np.mean((feature_pred - feature_true) ** 2)
            feature_mae = np.mean(np.abs(feature_pred - feature_true))
            ...
            print(f"Feature {feature_idx}: MSE={feature_mse:.4f}, MAE={feature_mae:.4f}")

        # Save results to CSV files
        # 1. Save per-sample detailed predictions
        detailed_results = []
            ...
            for t in range(predictions.shape[0]):
                for f in range(n_features):
                    detailed_results.append({
                        ...
                        'timestep': t,
                        'feature_index': f,
                        'prediction': predictions[t, f],
                        'target': targets[t, f],
                        'error': predictions[t, f] - targets[t, f],
                        'abs_error': abs(predictions[t, f] - targets[t, f])
                    })

        detailed_df = pd.DataFrame(detailed_results)
        detailed_df.to_csv(os.path.join(folder_path, "detailed_predictions.csv"), index=False)

        # 2. Save per-sample aggregated metrics
        sample_metrics = []
        for i, (pred, true) in enumerate(zip(all_predictions, all_targets)):
            sample_mse = np.mean((pred - true) ** 2)
            sample_mae = np.mean(np.abs(pred - true))

            feature_mse = []
            feature_mae = []
            for feature_idx in range(n_features):
                feature_mse.append(np.mean((pred[:, feature_idx] - true[:, feature_idx]) ** 2))
                feature_mae.append(np.mean(np.abs(pred[:, feature_idx] - true[:, feature_idx])))

            sample_metrics.append({
                'sample_id': i,
                'overall_mse': sample_mse,
                'overall_mae': sample_mae,
                **{f'feature_{j}_mse': feature_mse[j] for j in range(n_features)},
                **{f'feature_{j}_mae': feature_mae[j] for j in range(n_features)}
            })

        sample_metrics_df = pd.DataFrame(sample_metrics)
        sample_metrics_df.to_csv(os.path.join(folder_path, "sample_metrics.csv"), index=False)

        # 3. Save per-feature aggregated metrics
        feature_metrics_df = pd.DataFrame(feature_metrics)
        feature_metrics_df.to_csv(os.path.join(folder_path, "feature_metrics.csv"), index=False)

        # Update log file
        log_path = os.path.join(folder_path, "score.txt")
        with open(log_path, "a") as f:
            f.write(f"Overall metrics - MSE: {mse:.4f}, MAE: {mae:.4f}\n")
            f.write("Per-feature metrics:\n")
            for metric in feature_metrics:
                f.write(f"Feature {metric['feature_index']}: MSE={metric['mse']:.4f}, MAE={metric['mae']:.4f}\n")

        print(f"Detailed predictions saved to: {folder_path}")
        print(f"- detailed_predictions.csv: per-sample, per-timestep, per-feature predictions")
        print(f"- sample_metrics.csv: per-sample aggregated metrics")
        print(f"- feature_metrics.csv: per-feature aggregated metrics")
            ...
            with open(log_path, "a") as log_file:
                log_file.write(
                    "Epoch: {0}, Steps: {1}, Time: {2:.2f}s | Train Loss: {3:.7f}, Acc: {4:.4f}, F1: {5:.4f} | Vali Loss: {6:.7f}, Acc: {7:.4f}, F1: {8:.4f} | Test Loss: {9:.7f}, Acc: {10:.4f}, F1: {11:.4f}\n".format(
                        epoch + 1, len(train_loader), end_time - start_time, train_loss, train_acc, train_f1, vali_loss,
                        vali_acc, vali_f1, test_loss, test_acc, test_f1
                    )
                )
